<div class="modal fade" id="editOrderModal{{ $order->id }}" tabindex="-1">
    <div class="modal-dialog  modal-dialog-centered">
        <form action="{{ route('adminorder.update', $order) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="mb-4">
                     ...
                     <div class="mb-4">
                        ...
                        <select name="delivery_status" class="form-control" required>
                            <option value="for_pickup" {{ $order->delivery_status == 'for_pickup' ? 'selected' : '' }}>For Pickup</option>
                            <option value="for_delivery" {{ $order->delivery_status == 'for_delivery' ? 'selected' : '' }}>For Delivery</option>
                            <option value="is_ongoing" {{ $order->delivery_status == 'is_ongoing' ? 'selected' : '' }}>Is Ongoing</option>
                            <option value="is_upcoming" {{ $order->delivery_status == 'is_upcoming' ? 'selected' : '' }}>Is Upcoming</option>
                        </select>
                    </div>
