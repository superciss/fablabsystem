    ...
    public function index()
{
    $products = Product::with('category')
        ->whereHas('category', function($query) {
            $query->whereIn('name', ['Finished Product', 'Wholesale']);
        })
...
{
    $lowProducts = Product::whereNotNull('low_stock_threshold')
        ->whereColumn('stock', '<=', 'low_stock_threshold')
        ->get();
    ...
    return response()->json($lowProducts);
      ...
      public function index_notify()
{
    // Only products where stock <= low_stock_threshold
    $lowProducts = Product::whereNotNull('low_stock_threshold')
        ->whereColumn('stock', '<=', 'low_stock_threshold')
        ->get();
    ...
    return view('admin.notification.index_notification', compact('lowProducts', 'outOfStock'));
}
